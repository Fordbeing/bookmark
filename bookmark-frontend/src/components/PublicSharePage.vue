<template>
  <div class="public-share-page">
    ...
    <div v-else-if="shareData" class="share-content">
      ...
      <el-empty v-else description="此分类暂无书签" />
...
const checkShare = async () => {
  try {
    const result = await request.get(`/public/share/${shareCode.value}/check`);
    if (result.data) {
...
const loadShareContent = async () => {
  ...
  const result = await request.get(`/public/share/${shareCode.value}`, { params });
  if (result.data) {
